...
The aerial\_v9 dataset is assembled from three public sources: aerial.v1i (Roboflow, CC BY 4.0),
Aerial Vehicle Detection (MIT), and aerial.v3i (MIT). Assembling it required unifying the source
class taxonomies into a single 7-class schema — for example, \texttt{bicycle} + \texttt{motorcycle}
+ \texttt{person\_on\_bicycle} $\rightarrow$ \texttt{cycle}, and AVD's \texttt{small bus}
$\rightarrow$ \texttt{small-bus}. An earlier version of the dataset (V7) also incorporated
VisDrone2019; its noisy labels degraded person/cycle recall to $\sim$50\%, so it was discarded in
favor of the cleaner sources listed above. The final dataset contains 8,075 training, 2,224
validation, and 738 test images across 7 classes. See \texttt{docs/data\_cleaning\_report.md} for
...
Epochs & 200 (V8--V16), 160 (V17), 120 (V18--V20) \\
...
V10.0 & + CoordAtt + aerial\_v9\footnotemark[3] & 70.51\% & 49.33\% & +0.92\% \\
V12.0 & imgsz 640$\rightarrow$800\footnotemark[1] & 71.58\% & 50.27\% & +1.07\% \\
V12.2 & + cls 0.5$\rightarrow$0.7 (fine-tune) & 72.10\% & 50.43\% & +0.52\% \\
V14.0 & P3+P4 dual-head, imgsz=960\footnotemark[2] & 73.71\% & \textbf{52.65\%} & +1.61\% \\
\textbf{V16.0} & \textbf{P3+P4 + per-scale CoordAtt, 800} & \textbf{74.00\%} & 52.11\% & +0.29\% \\
V17.0 & + P4 RepNCSPELAN4 & 73.97\% & 51.56\% & $-$0.03\% \\
V18.0 & + ECA / ASFF & 73.46\% & 51.16\% & $-$0.51\% \\
V19.0 & P4 wide channel 256$\rightarrow$384 & 73.29\% & 50.89\% & $-$0.17\% \\
V20.0 & P3+P4+P5 triple-head (clean) & 73.29\% & 50.41\% & $-$0.71\% \\
\bottomrule
\end{tabular}
\end{table}

\footnotetext[1]{V12's jump also lowered batch from 8 to 4, forced by VRAM at imgsz=800 — the
two changed together, so +1.07\% is not a pure resolution ablation.}
\footnotetext[2]{V14 bundles five simultaneous changes (head removal, TAL threshold, class
weighting, augmentation scale, imgsz); see Section~\ref{sec:experiments} for the isolated
head-count comparison (V16 vs V20, +0.71\%).}
\footnotetext[3]{V8.0--V8.1 used the smaller aerial\_v8 set; V10.0 onward use aerial\_v9. The
V8.1$\rightarrow$V10.0 row therefore also changes dataset, not only the loss/attention modules.}
