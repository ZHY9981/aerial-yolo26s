...
% === Title ===
\title{\textbf{Resource-Constrained Aerial Small-Object Detection\\with YOLO26s and Coordinate Attention}}

\author{
    Haoyi (Yuki) Zou\\[2mm]
    \small Independent Research\\[1mm]
    \small \texttt{https://github.com/ZHY9981/Drone\_yolo\_7types}
}
...
triple-head by 2.1\% mAP under memory constraints.
\end{abstract}
...
The aerial\_v9 dataset exhibits extreme long-tail distribution: person (73,770 instances) and
car (69,840) dominate, while freight (1,306) and small-bus (1,117) each represent less than 2\%
...
Removing the P5 detection head reduces parameter count by 32\% (from 10.3M to 7.03M for YOLO26s)
and saves approximately 40\% of detection-head VRAM. This freed memory is reallocated to larger
P3/P4 feature channels, improving small-object feature quality.

The counterintuitive result is that the dual-head configuration \textit{improves} mAP by +2.1\%
over triple-head (V20 vs V16 in our ablations). This is not because P5 is useless—the P5 head
detects large objects (buses, freight trucks) effectively—but because under the 8 GB memory
constraint, the memory saved by removing P5 allows larger P3/P4 channels that improve the
detection of the majority of targets (person, cycle, car, truck), which collectively account
for over 80\% of instances.
...
\subsection{TAL Positive-Sample Threshold}

The default TaskAlignedAssigner (TAL) \cite{feng2021toad} uses a stride-tensor minimum of 8 pixels
for positive-sample assignment. At P3 resolution (stride 8), this means any ground-truth center
more than 8 pixels from an anchor point is labeled negative. For a 20-pixel pedestrian, this
excludes over 80\% of valid P3 anchor points from receiving positive gradients.

We reduce the threshold to 4 pixels. This gives small targets approximately 4$\times$ more
positive anchors per instance, directly improving recall on person and cycle by +6--7\% each.

\subsection{Class-Weighted Loss}

We apply per-class multiplicative weights to the classification loss:

\begin{equation}
    \mathbf{w} = [2.0, 3.0, 1.8, 1.5, 1.0, 1.0, 1.0]
\end{equation}

corresponding to \texttt{[person, cycle, bus, small-bus, car, truck, freight]}. Cycle receives
the highest weight (3.0$\times$) because it is both rare (14K instances) and small. Freight and
small-bus, while rare, are large and easy to detect, so they use base weight.
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Dual-head outperforms triple-head on 8 GB.} Removing P5 saves 32\% parameters
          and improves mAP by +2.1\%. The memory saved enables larger P3/P4 channels that
...
largest opportunity for improvement given car's high instance count (69,840). Current hypotheses:
...
\emph{arXiv preprint arXiv:2301.10051}, 2023.
